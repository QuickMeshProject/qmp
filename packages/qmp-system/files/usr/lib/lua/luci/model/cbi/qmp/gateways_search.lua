...
-- Get the names of the search gateways
gw_names = {}
...
-- Display all the gateways to search for
for _,gw in ipairs(gw_names) do
  gw_section = m:section(NamedSection, gw, 'gateway', 'Search for gateway "' .. gw .. '"')
...
new_gw_section = m:section(TypedSection, "new_search_gateway",
translate("Add new gateways and routes to search for"),
translate("Use this option to add new gateways this qMp node will search for and fetch from the mesh network (e.g., to look for an Internet gateway, to fetch specific subnetwork announcements).")
.."<br/>"
..translate('Use a <strong>unique name</strong> for the new gateway section, which is not being used neither in this tab or in the "Offered gateways" tab.'))
...
function m.on_commit(self,map)
  ...
  ngw:foreach("gateways", "new_search_gateway", function(n)
    local nname = n[".name"]
    ...
    do
      ...
      ogw:foreach("gateways", 'gateway', function (o)
        local oname = o[".name"]
        local nname_dup = nname..dupname
        if (nname_dup == oname) then
          this_dup = true
        end
      end)
      dup = this_dup
    ...
    ogw:set("gateways", nname..dupname, "gateway")
    ogw:set("gateways", nname..dupname, "ignore",
      tgw:get('gateways', nname, 'ignore'))
    ogw:set("gateways", nname..dupname, "type",
      tgw:get('gateways', nname, 'type'))
    ogw:set("gateways", nname..dupname, "network",
      ...
      for _,op in ipairs(gwoptions) do
        if op.name ~= nil then
          ...
          if opvalue ~= nil then
            ogw:set("gateways", nname..dupname, op.name, opvalue)
          end
        end
      end
