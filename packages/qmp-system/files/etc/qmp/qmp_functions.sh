...
if [ -f /lib/bmx7_topology.so ]; then
  uci set $conf.bmx7_topology_plugin=plugin
  uci set $conf.bmx7_topology_plugin.plugin=bmx7_topology.so
fi
  ...
  for dev in $(qmp_get_devices mesh); do
    echo "Configuring interface $dev in BMX7"
  ...
  if qmp_uci_test qmp.networks.bmx7_ripe_prefix48 ; then
    uci set $conf.general.tun6Address="$(uci get qmp.networks.bmx7_ripe_prefix48):$device_id:0:0:0:1/64"
    uci set $conf.tmain=tunDev
    uci set $conf.tmain.tunDev=tmain
    uci set $conf.tmain.tun6Address="$(qmp_uci_get networks.bmx7_ripe_prefix48):$device_id:0:0:0:1/64"
  fi
